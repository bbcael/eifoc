...
h = Global_Rrs;
wvlend = 251; % use 400-650nm -- n.b. wavelengths above 648nm are <0.0003 Sr-1 >95% of the time, which is the 1km x 1km pixel absolute error for PACE
h = h(:,1:wvlend);
...
p1 = plot(1:modeend,ph(1:modeend),'linewidth',2,'color',[87 104 81]./256); % make scree plot
hold on; box on; 
scatter(1:modeend,ph(1:modeend),100,[87 104 81]./256,'filled')
p2 = plot(1:modeend,pm(1:modeend),'linewidth',2,'color',[149 83 62]./256);
p4 = plot(1:modeend,ps(1:modeend),'linewidth',2,'color',[128 40 97]./256);
scatter(1:modeend,ps(1:modeend),100,[128 40 97]./256,'filled')
...
axis([0.5 (modeend+.5) 0 85]);
xlabel('component','fontsize',16,'interpreter','latex');
ylabel('percent of variance explained','fontsize',16,'interpreter','latex');
lgnd = legend([p1 p2 p4 p3],'hyperspectral (58, 35, 5)','MODIS (64, 29, 6)','SeaWiFS (79, 20)','significance threshold');
set(lgnd,'interpreter','latex','fontsize',15,'location','northeast')
title('PCA: 191 global spectra, standardised, 400-650nm','fontsize',16,'interpreter','latex')
...
for i = 1:251; % predict each wavelength from multispectral equivalents via linear regression
...
p1 = plot(400:650,rmse_m,'linewidth',2,'color',[149 83 62]./256);
hold on;
p2 = plot(400:650,rmse_s,'linewidth',2,'color',[128 40 97]./256);
p3 = plot(400:650,.05.*nanmean(h),'--','linewidth',2,'color',[87 104 81]./256); % 5\% significance level
box on; 
axis([400 650 0 7e-5]);
xlabel('wavelength [nm]','fontsize',16,'interpreter','latex');
ylabel('sr$^{-1}$','fontsize',16,'interpreter','latex');
set(gca,'ticklabelinterpreter','latex','fontsize',15)
lgnd = legend([p1 p2 p3],'RMSE of MODIS regression','RMSE of SeaWiFS regression','5\% of mean hyperspectral Rrs');
set(lgnd,'interpreter','latex','fontsize',15,'location','northeast')
...
p1 = plot(1:6,pr,'linewidth',2,'color',[87 104 81]./256); % make scree plot
hold on; box on; 
scatter(1:6,pr,100,[87 104 81]./256,'filled')
p2 = plot(1:6,pp(1:6),'linewidth',2,'color',[149 83 62]./256);
p3 = plot(1:6,bs6,'k','linewidth',1.25);
scatter(1:6,pp(1:6),100,[149 83 62]./256,'filled')
set(gca,'ticklabelinterpreter','latex','fontsize',16,'xtick',1:6)
axis([0.5 6.5 0 60]);
xlabel('mode number','fontsize',16,'interpreter','latex');
ylabel('percent of variance explained','fontsize',16,'interpreter','latex');
